...
export default function ResultScreen({ result, unitLabel, onRetry, onBack, mode }) {
  ...
  return (
    <div className="result-screen">
      {isTest && (
        <div className={`test-result-banner ${isPerfect ? 'test-pass' : 'test-fail'}`}>
          {isPerfect ? '合格' : '不合格'}
        </div>
      )}

      <div className="result-hero">
        <div className="result-msg">{isTest ? (isPerfect ? '全問正解！おめでとう！' : 'あと少し！もう一度挑戦！') : msg}</div>
      ...
      <div className="result-actions">
        <button className="retry-btn" onClick={onRetry}>{isTest && !isPerfect ? '再テスト' : 'もう一度'}</button>
